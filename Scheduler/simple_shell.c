...
void cleanup_and_exit() {
    if (sched_pid > 0) {
        // Give scheduler a short window to pick up any recently-submitted jobs.
        int waited_ms = 0;
        ...
        // Now request scheduler to terminate
        kill(sched_pid, SIGTERM);

        // give scheduler a short moment to clean up current slice and reap children
...
int main(int argc, char *argv[]) {
    ...
    printf("Commands: submit <path>, exit, help\n\n");
